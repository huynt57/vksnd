...
class DocumentaryAbroadController extends Controller
{
    ...
    public function actionIndex() {
        $data = DocumentaryAbroad::model()->getDocumentary();
    ...
    public function actionAddProcess() {
        try {
            ...
            $result = DocumentaryAbroad::model()->add($_POST);
            if ($result) {
                Yii::app()->user->setFlash('success', 'Thêm dữ liệu thành công !');
                $this->redirect(Yii::app()->createUrl('documentaryAbroad/add'));
            } else {
    ...
    public function actionDelete() {
        ...
        $result = DocumentaryAbroad::model()->findByPk($id);
        if ($result->delete()) {
            Yii::app()->user->setFlash('success', 'Xóa dữ liệu thành công !');
            $this->redirect(Yii::app()->createUrl('documentaryAbroad/index'));
        } else {
    ...
    public function actionEditProcess() {
        try {
            $post = StringHelper::filterArrayString($_POST);
            $result = DocumentaryAbroad::model()->edit($_POST);
            if ($result == 1) {
                Yii::app()->user->setFlash('success', 'Cập nhật dữ liệu thành công !');
                $this->redirect(Yii::app()->createUrl('documentaryAbroad/edit', array('id' => $post['id'])));
            } else if ($result == 2) {
