...
Yii::import('application.models._base.BaseDocumentaryAbroad');

class DocumentaryAbroad extends BaseDocumentaryAbroad {
    ...
    public function getDocumentary() {
        ...
        $models = DocumentaryAbroad::model()->findAll($criteria);
        return array(
            'models' => $models,
            'pages' => $pages
        );
    ...
    public function edit($post) {
        $doc = DocumentaryAbroad::model()->findByPk($post['id']);
        if ($doc) {
...
//            $criteria->addBetweenCondition('date_prosecution', $date_start, $date_end);
        ...
        if ($pagination != 1) {
            $result = DocumentaryAbroad::model()->findAll($criteria);
            return $result;
        }
        $pages = new CPagination($count);
